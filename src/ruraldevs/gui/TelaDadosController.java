...
public class TelaDadosController implements Initializable {
	...
	public void carregarGraficos() {
		try {
			...
			List<RegistroCasos> listaRegistrosCasos = mainTelas.registrosCasosController.filtrar(cbEstados.getValue(),
					cbCidades.getValue(), datepInicial.getValue(), datepFinal.getValue());
			for (RegistroCasos registroCasos : listaRegistrosCasos) {
				novosCasosSeries.getData().add(new XYChart.Data<String, Number>(
						formatter.format(registroCasos.getData()), registroCasos.getNumeroDeNovosCasos()));
				novosCasosSeries.setName("Número de casos");
				totalCasosSeries.getData().add(new XYChart.Data<String, Number>(
						formatter.format(registroCasos.getData()), registroCasos.getNumeroTotalDeCasos()));
				totalCasosSeries.setName("Número total de casos");
				novasMortesSeries.getData().add(new XYChart.Data<String, Number>(
						formatter.format(registroCasos.getData()), registroCasos.getNumeroDeNovasMortes()));
				novasMortesSeries.setName("Número de mortes");
				totalMortesSeries.getData().add(new XYChart.Data<String, Number>(
						formatter.format(registroCasos.getData()), registroCasos.getNumeroTotalDeMortes()));
			...
			lblLastDate.setText(String.format("Última informação recebida na data: %s",
					formatter.format(this.getLastDate(listaRegistrosCasos))));
	...
	public void preencherCidades(ActionEvent event) {
		...
		cbCidades.getItems().addAll(mainTelas.estadosController.getCidadesPorEstado(estado));
	}
